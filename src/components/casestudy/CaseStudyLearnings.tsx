...
export const CaseStudyLearnings: React.FC<CaseStudyLearningsProps> = ({
  ...
  return (
    <section aria-labelledby="retrospective-heading" className={cn('flex flex-col gap-6 py-10 border-b border-border', className)}>
      <div className="flex items-center gap-2 text-xs font-mono text-accent font-semibold uppercase tracking-wider">
        <Sparkles className="w-3.5 h-3.5 text-accent" aria-hidden="true" />
        <h2 id="retrospective-heading" className="text-xs font-mono text-accent font-semibold uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {learnings.map((learning, index) => (
          ...
          >
            <span className="font-mono text-xs text-muted-subtle font-bold" aria-hidden="true">
              0{index + 1}
            </span>
